...
pub fn run(
    ...
    const normalized_target = try normalizeTargetPath(allocator, lf.root_path, cwd_path, raw_target);
    defer allocator.free(normalized_target);

    var printed: std.StringHashMap(void) = std.StringHashMap(void).init(allocator);
